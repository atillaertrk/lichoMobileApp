        ...
        NavigationStack{
            ZStack{
                ...
                VStack{
                    ...
                    HStack{
                        TextField("Other ", text: $ViewModel.other).padding(6).multilineTextAlignment(.center).border(.softBlack).background(RoundedRectangle(cornerRadius: 10).stroke(.softBlack)).cornerRadius(10).padding(.leading, 20).keyboardType(.decimalPad).onTapGesture {
                    ...
                    VStack{
                        ...
                        Divider()
                        Image("waterTarget").resizable().frame(width: 70, height : 70).padding(5).foregroundStyle(.red)
                        VStack{
                            HStack {
                                Text("Amount of water consumed").foregroundStyle(.softBlack)
                                Spacer()
                                if profile?.volumeUnit ?? "ml" == "ml" {
                                    Text("\(String(format: "%.2f", ViewModel.totalConsumedML)) \(profile?.volumeUnit ?? "ml")").foregroundStyle(.softBlack)
                                } else {
                                    Text("\(String(format: "%.2f", ViewModel.totalConsumedFloz)) \(profile?.volumeUnit ?? "ml")").foregroundStyle(.softBlack)
                                }
                                
                            }
                            HStack{
                                Text("Reach your goal").foregroundStyle(.softBlack)
                                ...
                                if profile?.volumeUnit ?? "ml" == "ml"{
                                    if (Double(ViewModel.waterGoal) ?? 2000) - (ViewModel.totalConsumedML) < 0 {
                                        ...
                                        Text("\(String(format: "%.2f", (Double(ViewModel.waterGoal) ?? 2000) - (ViewModel.totalConsumedML))) \(profile?.volumeUnit ?? "ml")").foregroundStyle(.softBlack)
                                    }
                                } else {
                                    if (Double(ViewModel.waterGoal) ?? 2000) - (ViewModel.totalConsumedFloz) < 0 {
                                        Text("0").foregroundStyle(.softBlack)
                                    } else {
                                        Text("\(String(format: "%.2f", (Double(ViewModel.waterGoal) ?? 2000) - (ViewModel.totalConsumedFloz))) \(profile?.volumeUnit ?? "ml")").foregroundStyle(.softBlack)
                                    }
                                }
                            ...
                            HStack{
                                ...
                                if profile?.volumeUnit ?? "ml" == "ml" {
                                    if (Double(ViewModel.waterGoal) ?? 2000) - (ViewModel.totalConsumedML) < 0 {
                                        Text("You reached your goal").foregroundStyle(.softBlack)
                                        Spacer()
